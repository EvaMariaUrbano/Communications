...
\end{center}
\end{figure}
\paragraph{}However, it is rare for a client satellite to only have one network satellite in range. It would need to be very close to the network satellite orbit, or to have an antenna with very narrow radiation pattern. For antennas with almost half-spherical patterns (an angle of 10º over their horizontal plane has been considered as the minimum angle capable of recieving and transmitting), the minimum height over the satellite network in order to see more than one satellite is, aproximately, 70 km, considering that our constellation is at 500 km height over the Earth's surface. This means that the majority of clients would have in range more than one satellite. For those clients, the critical failure would be similar as the ones in Figure \ref{fig:critical2} and Figure \ref{fig:critical3}. In the case of client satellites whithin the 70 km distance, and because of the little distance between client and network satellite, it is strongly recommended the use of isotropic ratiator antennas, which will allow to see more network satellites.
\begin{figure}[H]
\begin{center}
...
\paragraph{}As it can be seen, the critical failure depends on the communication range of the client satellite. Taking the more restrictive one would mean considering the failure of only one satellite. However, it has been stated that the use of isotropic radiation antennas is higly recommended for client satellites that are at less than 70 km from the network orbits. Therefore, this critical failure will not be considered. Instead, it will be considered the case of three network satellites. For a critical failure to happen, at least three adjacent satellites must fail. There is no time requiement for this criteria, since it does not matter at which time they fail, but the fact that the trhee of them are fallen at a given time.
...
\begin{itemize}
\item The failure of at least three adjacent satellites.
